...
func Guide(conf GuideConf) Actor {
	...
	t1 := Train{
		Power:        60,
		CurrentBack:  1, // FullPathTo returns a path where index 0 is the starting port of the first line, and index 1 is the exiting port of the first line.
		CurrentFront: 1,
	...
	t1.Path = g.y.MustFullPathTo(LinePort{g.y.MustLookupIndex("W"), layout.PortB}, LinePort{g.y.MustLookupIndex("Z"), layout.PortA})
	...
	for diffuse := range g.actor.InputCh {
		...
		case GuideTrainUpdate:
			...
			if backSame != frontSame {
				backA := val.Train.Path[val.Train.CurrentBack]
				backB := orig.Path[orig.CurrentBack]
				frontA := val.Train.Path[val.Train.CurrentFront]
				frontB := orig.Path[orig.CurrentFront]
